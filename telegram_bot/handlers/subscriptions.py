...
from keyboards.subscription import (manage_new_sub_trial_keyboard,
                                    new_sub_keyboard)
...
async def cb_new_sub(callback: CallbackQuery, session, state: FSMContext):
...
async def cb_manage_sub(callback: CallbackQuery, session, state: FSMContext):
    ...
    if message is None:
        return
    manage_user = ManageUser(user_tg_id=callback.from_user.id,
...
async def callback_new_sub(callback: types.CallbackQuery, state: FSMContext,
    ...
    if user_info == 409:
        ...
        await message.edit_text(f"У вас уже есть подписка:\n<code>{subs_user[0]['sub_link']}</code>", parse_mode="HTML", reply_markup=kb_cancel())
    elif not user_info:
